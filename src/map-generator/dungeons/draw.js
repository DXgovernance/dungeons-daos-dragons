...
function drawRoomPillars({ gridX, gridY, gridWidth, gridHeight }) {
  ...
  pillars += drawPillarCell({
    gridX: gridX + pillarGridInset,
    gridY: gridY + pillarGridInset,
  });
  pillars += drawPillarCell({
    gridX: gridX + innerWidth,
    gridY: gridY + pillarGridInset,
  });
  pillars += drawPillarCell({
    gridX: gridX + pillarGridInset,
    gridY: gridY + innerHeight,
  });
  pillars += drawPillarCell({
    gridX: gridX + innerWidth,
    gridY: gridY + innerHeight,
...
function drawText(
  text,
  { x, y },
  { fontSize = fontSizeNormal, fill = colorText } = {}
) {
  let attributes = {
    x,
    y: y + pxTextOffset,
...
export function drawDoor(gridRectangle, { direction, type, locked }) {
  ...
  if (lockable.has(type)) {
    ...
    details.push(
      drawCircle({ cx, cy, r: radiusHole }, { fill: colorPillarFill })
    );
  ...
  let lines = lineCords
    .map(cords => drawLine({ ...cords, ...lineAttrs }, lineOptions))
    .join('');
