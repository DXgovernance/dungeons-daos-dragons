...
contract ERC20Guild is Initializable, IERC1271Upgradeable {
  ...
  function initialize(
    ...
    callPermissions[address(this)][
      bytes4(keccak256('setEIP1271SignedHash(bytes32,address)'))
    ] = true;
    initialized = true;
  }
  ...
  function setEIP1271SignedHash(bytes32 _hash) public virtual {
    require(
      msg.sender == address(this),
      'ERC20Guild: Only callable by the guild'
    );
    EIP1271SignedHashes[_hash] = true;
  }
  ...
  function createProposals(
    ...
    for (
      ...
      uint256 callIndex;
      for (
        uint256 callIndexInProposals = callsPerProposal.mul(proposalIndex);
  ...
  function _createProposal(
    ...
    bytes32 proposalId =
      keccak256(abi.encodePacked(msg.sender, block.timestamp, proposalNonce));
  ...
  function votesOf(address[] memory accounts)
    public
    view
    virtual
  ...
  function votesOfAt(address account, uint256 snapshotId)
    public
    view
    virtual
    returns (uint256)
  ...
  function getProposalVotes(bytes32 proposalId, address voter)
    public
    view
    virtual
    returns (uint256)
  ...
  {
    return
      ((votesOf(hash.recover(signature)) > 0) && EIP1271SignedHashes[hash])
